    ...
    <div id={`material-row-${id}`} className={`flex flex-col gap-2 p-3 rounded-xl border transition-colors ${isDone ? 'border-green-400 dark:border-green-600/40 bg-green-50 dark:bg-green-950/20' : 'border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50'}`}>
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <button
            {...dragHandleProps}
            className="p-1 cursor-grab active:cursor-grabbing text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400 touch-none shrink-0"
            aria-label="순서 변경"
          >
      ...
      <div className="flex items-start justify-between text-xs px-0.5">
        <span className={`flex flex-wrap gap-x-1 ${isDone ? 'text-green-600 dark:text-green-500' : 'text-gray-400 dark:text-gray-500'}`}>
          <span>보유 {current_count.toLocaleString()}개</span>
          <span>({toSets(current_count)})</span>
        </span>
        <span className="flex flex-wrap gap-x-1 justify-end text-right text-gray-400 dark:text-gray-600">
          <span>필요 {required_count.toLocaleString()}개</span>
          <span>({toSets(required_count)})</span>
        </span>
      </div>
